...
import { getOAuthInstallUrl, disconnectOAuth } from '@/lib/api/shopify-oauth';
...
export default function SettingsPage() {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      {/* SaaS-style integration overview */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-5 shadow-sm">
          <div className="flex items-start justify-between">
            ...
            <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${
              whatsappIntegration ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-700'
            }`}>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 sm:p-5 shadow-sm">
          <div className="flex items-start justify-between">
            ...
            <span className={`px-2.5 py-1 rounded-full text-xs font-semibold ${
              shopifyStore ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-700'
            }`}>
              {shopifyStore ? 'Connected' : 'Not Connected'}
            </span>
          </div>
        </div>
      </div>
          ...
          <div className="p-4 sm:p-6 space-y-6">
            ...
            {shopifyStore && shopifyStore.tokenType === 'oauth' && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-6">
                ...
                <div className="bg-white border border-green-300 rounded-lg p-4 mt-4 space-y-3">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-600 mb-3">
                        Webhook registration is manual. Configure these once in Shopify Admin to keep order sync stable.
                      </p>

                      <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                        <p className="text-xs font-semibold text-blue-900 mb-2">Webhook Endpoint</p>
                        <div className="flex items-center gap-2">
                          <code className="flex-1 text-xs bg-white border border-blue-200 rounded px-2 py-1.5 font-mono break-all text-blue-900">
                            {shopifyWebhookEndpoint}
                          </code>
                          <button
                            type="button"
                            onClick={() => {
                              navigator.clipboard.writeText(shopifyWebhookEndpoint);
                  ...
                  <div className="bg-amber-50 border border-amber-200 rounded p-3">
                    <p className="text-xs text-amber-900 font-semibold mb-2">Manual Setup Steps</p>
                    <ol className="text-xs text-amber-800 ml-4 space-y-1 list-decimal">
                      <li>Open Shopify Admin → Settings → Notifications → Webhooks</li>
                      <li>Create webhook with format JSON and API version latest stable</li>
                      <li>Use the endpoint above for each event</li>
                      <li>Add these topics: orders/create, orders/updated, orders/cancelled</li>
                      <li>Save and send a test webhook to verify delivery</li>
                    </ol>
                  </div>

                  <div className="bg-green-50 border border-green-200 rounded p-3">
                    <p className="text-xs text-green-800">
                      ✅ Webhooks are one-time configuration. Reconfigure only if your public URL changes or app is reinstalled.
                    </p>
                  </div>
